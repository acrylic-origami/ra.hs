...
import Ra ( pat_match )
import Ra.Stack ( Sym(..), SymTable(..), StackBranch, merge_sym_tables, mksym )
import Ra.Extra
...
bind_to_table branch = \case
  ...
  PatBind { pat_lhs = L _ pat_lhs, pat_rhs } ->
    let branch' = update_head (second (merge_sym_tables . (:[grhs_binds pat_rhs]))) branch
        next_exprs = grhs_exprs pat_rhs
        next_tables = map (\expr -> pat_match branch' expr pat_lhs) (map mksym next_exprs)
    in merge_sym_tables next_tables
...
grhs_binds = merge_sym_tables . everythingBut (++) (
