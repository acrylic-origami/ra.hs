...
module Rahse (
  ...
  pat_match MatchGroup{ mg_alts } args = map unions $ transpose $ map (map (pat_match . unLoc) . zip args . m_pats . unLoc . m_pats) mg_alts where -- refactor; there has to be a more elegant way to deal with these dimensions
    ...
    pat_match' ret (ConPatOut { pat_con, pat_args }) =
      let con_name :: String
          ...
          decompose = foldl1 (liftA2 union) . map (uncurry pat_match') . zip (rt_args ret) . map unLoc -- narrowing, could use assert
      in if con_name == nameString $ varName $ rt_name $ rt_left ret -- narrowing
