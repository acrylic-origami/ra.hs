...
pat_multi_match ::
  ...
  -> Sym
  -> SymTable
pat_multi_match expand branch pats expr =
...
pat_match branch pat expr = case pat of
  ...
  NPat _ _ _ _ -> empty
  -- container
  ListPat pats _ _ -> union_sym_tables $ map (\(L _ pat') -> pat_match branch pat' expr) pats -- encodes the logic that all elements of a list might be part of the pattern regardless of order
  AsPat (L _ bound) (L _ pat') -> union_sym_tables [
      singleton bound [expr]
      , pat_match branch pat' expr
...
reduce_deep branch args expr =
  let terminal =
        ...
        ((pure *** ((++args) . map pure)) $ deapp expr)
        & (uncurry $
            foldl (\ress args' -> [HsApp (noLoc res) (noLoc arg) | res <- ress, arg <- concatMap (reduce_deep branch []) args' ])
  ...
  in case expr of
    ...
    HsLam mg | let loc = getLoc $ mg_alts mg -- <- NB this is why the locations of MatchGroups don't matter
      ...
      if matchGroupArity mg > length args
        then terminal
          ...
          in concatMap (reduce_deep next_branch next_args) next_exprs
    ...
    HsVar (L _ v) | Just left_exprs <- branch_lookup v branch ->
      ...
      in concatMap (reduce_deep branch args) left_exprs -- TODO The whole StackBranch structure is a little screwy, because all of them should eventually lead to lambdas except for unresolvable bindings. Therefore, 
    ...
    SectionR (L _ m_op) (L _ v) ->
      ...
      in case branch_lookup op branch of
        Just branch_exprs -> branch_exprs & foldr (\(HsLam mg) -> (++(reduce_deep branch ([v] : args) (HsLam (mg_flip mg))))) [] -- BOOMARK: also do the operator constructor case
        Nothing -> terminal
    ...
    HsMultiIf ty rhss ->
      ...
      in concatMap (reduce_deep (next_frame : branch) args) next_exprs
      -- (HsLam (MatchGroup {
