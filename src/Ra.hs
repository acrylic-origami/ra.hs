...
-- Valid Pat:
pat_match branch sym@(Sym (m_parts, expr)) pat = case pat of
  -- empty
  WildPat ty ->
  ...
  LitPat _ -> SymTable empty -- no new name bindings
  NPat _ _ _ _ -> SymTable empty
  -- container
...
reduce_deep _ (Sym (_, HsConLikeOut _)) args@(_:_) = error "Only bare ConLike should make it to `reduce_deep`" -- $ (map constr_ppr args) ++ 
...
reduce_deep branch sym@(Sym (m_parts, expr)) args =
  let terminal =
        assert (isNothing m_parts) $
          map (Sym . (m_parts,)) $ foldl (\ress args -> [HsApp (noLoc res) (noLoc arg) | res <- ress, Sym (_, arg) <- args ]) [expr] args -- TODO fishy information loss here on the Sym -> expr conversion; we may need to keep the bindings from any partials, even if it's terminal. This might be temporary anyways
      unravel1 v = reduce_deep branch (Sym (m_parts, v)) args
      unravel :: [HsExpr Id] -> [Sym]
      unravel = concatMap (flip (reduce_deep branch) args . Sym . (m_parts,))
  ...
  in case expr of
    ...
    HsApp _ _ ->
      let (fun, next_args) = deapp expr
          passthrough = reduce_deep branch (mksym fun) (map (pure . mksym) next_args ++ args)
    ...
    SectionR (L _ m_op) (L _ v) ->
      ...
      in case branch_lookup op branch of
        Just branch_exprs -> branch_exprs & foldr (\(Sym (m_table, HsLam mg)) -> (++(reduce_deep branch (Sym (m_table, HsLam (mg_flip mg))) ([Sym (m_parts, v)] : args)))) [] -- BOOMARK: also do the operator constructor case
        Nothing -> terminal
    ...
    HsCase l_x@(L _ x) mg -> reduce_deep branch (Sym (m_parts, HsApp (L (getLoc $ mg_alts mg) (HsLam mg)) l_x)) args -- refactor as HsLam so we can just use that pat match code
