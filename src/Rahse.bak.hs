...
module Rahse (
  ...
  data DataFields = Args [[ReturnThing]] | Recs (Map GhcTc [ReturnThing]) -- look at `HsConPatDetails` for the type of the record label name for the map key
  
  
	data ReturnThing =
		Var { rt_name :: GhcTc } | -- we only need this because the Var might be bound to a pattern (or of course an imported value from a package)
    ...
    Lit { rt_def :: HsLit } |
    Lam { rt_def :: MatchGroup GhcTc (HsExpr GhcTc) } -- needed because named function vars aren't 
    Case { rt_arg :: [ReturnThing], rt_def :: MatchGroup }
    
    -- Data { rt_fields :: DataFields } | -- omitting record args from v1
  
  type SymTable = Map GhcTc (Maybe MatchGroup, [ReturnThing])
  ...
  mg_cov_ids :: MatchGroup GhcTc (HsExpr GhcTc) -> [ReturnThing]
  ...
  stmt_cov_ids :: Stmt GhcTc (HsExpr GhcTc) -> [ReturnThing]
  stmt_cov_ids (BindStmt _ b _ _ _) = cov_ids b
  stmt_cov_ids (BodyStmt b _ _ _) = cov_ids b

  reduce_funs :: [ReturnThing] -> [ReturnThing]

	cov_ids :: HsExpr GhcTc -> [ReturnThing] -- right-to-left unapplied covariant bindings
  ...
  from_section :: HsExpr GhcTc -> HsExpr GhcTc -> [ReturnThing]
  ...
  pat_match :: MatchGroup GhcTc (HsExpr GhcTc) -> [[ReturnThing]] -> SymTable
  ...
  pat_match MatchGroup{ mg_alts } args = map unions $ transpose $ map (map (pat_match' . unLoc) . zip args . m_pats . unLoc . m_pats) mg_alts where -- refactor; there has to be a more elegant way to deal with these dimensions
    pat_match' :: ReturnThing -> Pat GhcTc -> Maybe SymTable
    -- wrapping patterns
    ...
    pat_match' (App{ rt_left, rt_args }) (ConPatOut{ pat_con, pat_args }) =
      let con_name :: String
          ...
          decompose :: [LPat GhcTc] -> Maybe SymTable
